...
function parseMeta(meta) {
  if (meta.startsWith("|meta=")) {
    meta = meta.slice(6) || [];
    try {
      return import_json5.default.parse(meta) || {};
    } catch {
      return {};
    }
  }
...
function parseDecorations(declaration) {
  if (declaration.startsWith("|decorations=")) {
    ...
    try {
      let parsed = import_json5.default.parse(declaration);
...
function markedCodeMoviePlugin({
  adapter,
  languages,
  addRuntime = false
}) {
    ...
    extensions: [
      {
        ...
        tokenizer(src) {
          const rule = /^`{4}code-movie\|(?<lang>[a-z-]+)(?<meta>\|meta={.*?}(?=\s+```))?(?<content>.*)`{4}/s;
        ...
        renderer(token) {
          ...
          if (addRuntime) {
            const controlsAttr = typeof addRuntime === "object" && addRuntime.controls ? ' controls="controls"' : "";
...
function markedCodeMovieHighlightPlugin({ adapter, languages }) {
  return {
    extensions: [
      {
        ...
        tokenizer(src) {
          const rule = /^`{4}code-movie-highlight\|(?<lang>[a-z-]+)?(?<decorations>\|decorations=.*?)?\n(?<content>.*)`{4}/s;
          const match = rule.exec(src);
          if (!match) {
            return;
          }
...
export {
  markedCodeMovieHighlightPlugin,
